...
import { SectionExclusiveContent } from "./types";

interface ExclusiveMaterialSectionProps {
  section_exclusive_contents: SectionExclusiveContent[];
}

export function ExclusiveMaterialSection({
  section_exclusive_contents,
}: ExclusiveMaterialSectionProps) {
  const [isPdfOpen, setIsPdfOpen] = useState(false);
  const [selectedMaterial, setSelectedMaterial] =
    useState<SectionExclusiveContent | null>(null);
  const [pdfUrl, setPdfUrl] = useState<string | null>(null);
  const [isLoadingPdf, setIsLoadingPdf] = useState(false);
  const { getLibraryFileForCourseByLibraryID } = useFileUpload();
  const defaultLayoutPluginInstance = defaultLayoutPlugin();

  const handleMaterialClick = async (material: SectionExclusiveContent) => {
  ...
  const renderMaterialList = () => (
    <div className=" w-full pr-4">
      {section_exclusive_contents.map((material: SectionExclusiveContent) => (
        <div
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 mt-1 text-sm text-gray-500 dark:text-gray-400">
                  <span>{material.library_item_id}</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <>
      ...
      <Dialog open={isPdfOpen} onOpenChange={setIsPdfOpen}>
        <DialogContent className="w-screen h-screen max-w-none m-0 p-6">
          <DialogHeader>
            <DialogTitle>{selectedMaterial?.library_item_id}</DialogTitle>
            <DialogDescription>
